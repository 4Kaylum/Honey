...
def add_output_args():
    ...
    def inner_wrapper(func):
        ...
        async def wrapper(request:Request):
            ...
            og_data['og:title'] = og_data.get('og:title', 'Honey - A furry-based Discord bot')
            og_data['og:description'] = og_data.get('og:description', 'Honey is a Discord bot used for furries')
            og_data['og:type'] = og_data.get('og:type', 'website')
            # og_data['og:image'] = og_data.get('og:image', 'https://marriagebot.xyz/static/images/HoneyCircle.150.png')
            og_data['og:locale'] = og_data.get('og:locale', 'en_GB')
            data['opengraph'] = og_data

            bot = request.app['bot']
            aiko = str(await bot.fetch_user(590794167362388011))
            callum = str(await bot.fetch_user(141231597155385344))
            names = {"Joshua": aiko, "Callum": callum}
            data.update(names)
